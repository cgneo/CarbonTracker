...
string replace(string& str, const string& from, long barcode){
...
{
    Food food;
    long barcode = food.get_barcode();

    const string url_("https://world.openfoodfacts.org/api/v0/product/[barcode].json");
    string url = replace(url, "[barcode]", barcode);
